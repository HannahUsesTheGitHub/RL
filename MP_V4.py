...
# NN architecture (for target and policy)
...
class ReplayBuffer:
    ...
    #changed sampling method to return the tensors
...
# Custom CartPole Environment with Random Pole Length
...
def worker_process(worker_id, policy_weights_queue, experience_queue, stop_flag, global_epsilon, episode_rewards):
    env = RandomPoleCartPole(min_length=0.4, max_length=1.8, randomize_each_episode=True)
...
def plot_results(avg_rewards):
    ...
    plt.title("Parallel DQN on Random-Pole CartPole-v1")
    plt.grid()
    plt.show()
...
if __name__ == "__main__":
    ...
    model_path = "weights/MP_DQN_random_pole.pth"
